...
function MinHWReq {
    ...
    if ($DiskSpace -gt 49) { 
        ...
        [System.Windows.Forms.MessageBox]::Show('Please free up more disk space. Minimum Requriments are 50 GB')
        timeout 10
    }
}
...
function BeginInstall {
    ...
    if ($null -eq $chromeInstalled.FileName) {
        ...
        Copy-Item C:\Install\Bookmarks "C:\Users\Administrator\AppData\Local\Google\Chrome\User Data\Default\Bookmarks"
    ...
    msiexec.exe /quiet /passive /qn /i "C:\Install\SQL\msodbcsql.msi" IACCEPTMSODBCSQLLICENSETERMS=YES ADDLOCAL=ALL
    ...
    try {
        ...
        [System.Windows.Forms.MessageBox]::Show('SQL Server was not installed correctly... Please contact Amendo Support')
    }
    ...
    [string]$GetDBVersion = sqlcmd -E -S .\sqlexpress -Q "USE ProTouch SET NOCOUNT ON;"

    If([string]$GetDBVersion -And $CK1) {
...
function Eierskifte {
    ...
    if (Test-Path -Path "C:\install\ProTouchBackup.bak") {
        ...
        [System.Windows.Forms.MessageBox]::Show('Ownership software changes completed! Please start ProTouch and enter new credentials.')
    }
    else {
